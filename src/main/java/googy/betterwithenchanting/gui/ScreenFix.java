...
public abstract class ScreenFix extends ScreenContainerAbstract {

	protected ScreenFix(MenuAbstract container) {
		super(container);
	}

	public abstract int getTargetSlot(ItemStack stackInSlot, int target);
	...
	public void clickInventory(int x, int y, int mouseButton) {
		...
		if (slotId != -1) {
			if (slotId == -999) {
				...
				if (stackInSlot != null) {
					int var10000 = stackInSlot.getItem().id;
				} else {
					boolean var30 = false;
				}
				...
				if (slot instanceof SlotResult) {
					if ((Boolean) GameSettings.SWAP_CRAFTING_BUTTONS.value) {
				...
				if (slot instanceof SlotCreative) {
					...
					/// ------------------------------------------------------------------------------------------------
					target = this.getTargetSlot(stackInSlot, target);
					...
					if (this.inventorySlots instanceof MenuFurnace) {
						...
						boolean isFuel = LookupFuelFurnace.instance.getFuelYield(stackInSlot) > 0;
						if (isIngredient) {
							target = 1;
						} else if (isFuel) {
							target = 2;
					...
					if (this.inventorySlots instanceof MenuFurnaceBlast) {
						boolean isIngredient = false;

						for (RecipeEntryBlastFurnace recipe : Registries.RECIPES.getAllBlastFurnaceRecipes()) {
							RecipeSymbol[] input = (RecipeSymbol[]) recipe.getInput();
							isIngredient = input[0].matches(stackInSlot);
							if (input.length == 2) {
								isIngredient |= input[1].matches(stackInSlot);
							}
						...
						boolean isFuel = LookupFuelFurnaceBlast.instance.getFuelYield(stackInSlot) > 0;
						if (isIngredient) {
							target = 1;
						} else if (isFuel) {
							target = 2;
					...
					if (this.inventorySlots instanceof MenuTrommel) {
						if (LookupFuelFurnace.instance.getFuelYield(stackInSlot) > 0) {
							target = 2;
						} else {
							target = 1;
						}
