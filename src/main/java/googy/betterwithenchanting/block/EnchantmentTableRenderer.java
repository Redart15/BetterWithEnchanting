...
public class EnchantmentTableRenderer extends TileEntityRenderer<TileEntityEnchantmentTable> {
	...
	private static final String TEXTURE_PATH = "/assets/" + BetterWithEnchanting.MOD_ID + "/textures/book/book.png";
	...
	public void doRender(Tessellator tessellator, TileEntityEnchantmentTable tileEntity, double x, double y, double z, float partialTicks) {
		...
		if (Minecraft.getMinecraft().gameSettings.items3D.value || (stack.getItem() instanceof ItemBlock && BlockModelDispatcher.getInstance().getDispatch(((ItemBlock) stack.getItem()).getBlock()).shouldItemRender3d())) {
		...
		renderer.render(tessellator, entityItem, 0D, -0.3D, 0D, 0F, 0F);
		GL11.glPopMatrix();
	}
}
